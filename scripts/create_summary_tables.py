...
With year support for multi-year data
"""
import os
...
def main():
    print("=" * 60, flush=True)
    print("Creating Summary Tables with Year Support", flush=True)
    print("=" * 60, flush=True)
    ...
    print(f"Available years in data: {available_years}", flush=True)
    ...
    # =========================================================
    # STEP 1: Create summary_monthly table with year column
    # =========================================================
    ...
    CREATE TABLE summary_monthly (
        ...
        UNIQUE KEY unique_country_year_month (country, year, month),
        INDEX idx_year (year)
    )
    """
    ...
    cursor.execute("""
        INSERT INTO summary_monthly (country, year, month, neg_hours, avg_market_price)
        WITH prices_raw AS (
            SELECT
                AreaCode,
                AreaDisplayName,
                `DateTime(UTC)`,
                YEAR(`DateTime(UTC)`) AS data_year,
                ResolutionCode,
                `Price[Currency/MWh]`,
                source_month,
                DATE_FORMAT(`DateTime(UTC)`, '%Y-%m-%d %H:00:00') AS hour_bucket
            FROM energy_prices
            WHERE
        ...
        hours_with_60m AS (
            SELECT DISTINCT AreaCode, hour_bucket
            FROM prices_raw
        ...
        prices_dedup AS (
            ...
            FROM prices_raw pr
            WHERE
                pr.ResolutionCode = 'PT60M'
                OR (
                    pr.ResolutionCode = 'PT15M'
                    AND NOT EXISTS (
                        SELECT 1 FROM hours_with_60m h60
                        WHERE h60.AreaCode = pr.AreaCode
                        AND h60.hour_bucket = pr.hour_bucket
                    )
                )
        )
        SELECT
        ...
        FROM prices_dedup
        GROUP BY AreaDisplayName, data_year, source_month
    """)
    conn.commit()
    ...
    cursor.execute("SELECT year, COUNT(*) FROM summary_monthly GROUP BY year ORDER BY year")
    for row in cursor.fetchall():
        print(f"   Year {row[0]}: {row[1]} rows", flush=True)
    ...
    if joined_count > 0:
        ...
        print("   3b. Calculating capture metrics per country/year/month...", flush=True)
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            JOIN temp_capture tc ON sm.country = tc.country AND sm.year = tc.year AND sm.month = tc.month
            SET 
    ...
    # =========================================================
    # STEP 4: Create summary_yearly table (aggregates all years per country)
    # =========================================================
    ...
    # =========================================================
    # STEP 6: Create summary_daily table with year column
    # =========================================================
    ...
    cursor.execute("""
        CREATE TABLE summary_daily (
            ...
            capture_rate DECIMAL(10,2) DEFAULT 0,
            solar_at_neg_price_pct DECIMAL(10,2) DEFAULT 0,
            UNIQUE KEY unique_country_year_month_day (country, year, month, day),
            INDEX idx_country_year_month (country, year, month),
            INDEX idx_year (year)
        )
    """)
    conn.commit()
    ...
    cursor.execute("""
        INSERT INTO summary_daily (country, year, month, day, neg_hours, avg_market_price)
        WITH prices_raw AS (
            SELECT
                AreaCode,
                AreaDisplayName,
        ...
        SELECT
            ...
            ROUND(AVG(`Price[Currency/MWh]`), 2) AS avg_market_price
        FROM prices_dedup
        GROUP BY AreaDisplayName, data_year, source_month, day_num
    """)
    conn.commit()
    ...
    # Show breakdown by year
    cursor.execute("SELECT year, COUNT(*) FROM summary_daily GROUP BY year ORDER BY year")
    for row in cursor.fetchall():
        print(f"   Year {row[0]}: {row[1]} rows", flush=True)
    ...
    if joined_count > 0:
        ...
        cursor.execute("""
            ...
            FROM joined
            GROUP BY country, year, month, day
        """)
        conn.commit()
        ...
        cursor.execute("""
            ...
            JOIN temp_daily_capture tc ON sd.country = tc.country AND sd.year = tc.year AND sd.month = tc.month AND sd.day = tc.day
            SET 
    ...
    row = cursor.fetchone()
    print(f"\nOverall Totals (All Zones, All Years):")
    ...
    # Show sample data per year
    print(f"\nSample monthly data per year:")
    cursor.execute("""
        SELECT year, country, month, neg_hours, avg_market_price 
        FROM summary_monthly 
        WHERE country = 'DE-LU'
        ORDER BY year, month
        LIMIT 6
    """)
    for row in cursor.fetchall():
        print(f"  {row[0]}/{row[2]:02d} {row[1]}: {row[3]} hrs, €{row[4]}")
